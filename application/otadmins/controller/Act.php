...
use app\otadmins\model\ActModel;
use think\Db;
...
//        }
        ...
        $limits = 15;// 获取总条数
        $count = $act->getAllCount($map);//计算总页面

        $allpage = intval(ceil($count / $limits));
        $lists = $act->getActByWhere($map, $Nowpage, $limits);
    ...
    public function edit()
    {
        ...
        if(request()->isAjax()){
            $param = input('post.');
            $time = $param['reservation'];
            $sldate=urldecode($time);//获取格式 2015-11-12 - 2015-11-18
            $arr = explode(" - ",$sldate);//转换成数组
            $start_time=strtotime($arr[0]);
            $end_time=strtotime($arr[1].' 23:55:55');
            if(empty($param['id'])){
                $data = [
                    'title' => $param['title'],
                    'sub_title' => $param['sub_title'],
                    'price' => $param['price'],
                    'user_brokerage' => $param['user_brokerage'],
                    'num' => $param['num'],
                    'vi_buy_num' => $param['vi_buy_num'],
                    'vi_uv' => $param['vi_uv'],
                    'vi_jion_num' => $param['vi_jion_num'],
                    'vi_share_num' => $param['vi_share_num'],
                    'share_type' => $param['share_type'],
                    'main_img' => $param['main_img'],
                    'post_img' => $param['post_img'],
                    'server_code' => $param['server_code'],
                ...
                $new_id = Db::name('act')->insertGetId($data);
                $d_data = [
                    ''
                ];
            }
        ...
        $act_id = input('param.id');
        $act = $actModel->getOneClassify(['id'=>$act_id]);
        $this->assign('act',$act);
        return $this->fetch();
    }
    /**
